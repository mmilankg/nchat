...
bool AcceptCallItem::action()
{
    callPopup->setResponse(0);
    return true;
}

bool RejectCallItem::action()
{
    callPopup->setResponse(1);
    return true;
}

CallPopup::CallPopup(const std::string & username, int width) :
    NCursesMenu(1, width, (lines() + 3) / 2, (cols() - width) / 2),
    response(0),
    panel(0),
    items(0)
{
    std::vector<std::string> buttonLabels = {"Accept", "Reject"};
    int                      nItems       = buttonLabels.size();
    int                      buttonLength = width / nItems;
    for (auto && label : buttonLabels)
        for (int i = label.size(); i < buttonLength; i++) label += " ";
    ...
    items    = new NCursesMenuItem *[nItems + 1];
    items[0] = new AcceptCallItem(this, buttonLabels[0]);
    items[1] = new RejectCallItem(this, buttonLabels[1]);
    items[2] = new NCursesMenuItem();
    ...
    panel->addstr(1, 6, username.c_str());
    panel->attroff(A_BOLD);
    panel->addstr(1, 6 + username.length() + 1, "initiated a voice call.");
    panel->show();
}
